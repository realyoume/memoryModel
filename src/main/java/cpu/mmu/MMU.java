...
public class MMU {
        ...
        if (!Memory.SEGMENT) {
            ...
            if (!Memory.PAGE) {
                ...
                for (int i = startvPageNo; i <= endvPageNo; i++) {
                    if (TLB.isAvailable) {
                        // TODO: add tlb here
                        if(!tlb.isInTlb(i) && !memory.isValidPage(i)){
                            memory.page_load(i);
                            tlb.write(i);
                        }else if(!tlb.isInTlb(i) && memory.isValidPage(i)){
                            tlb.write(i);
                        }
    ...
    private String toRealLinearAddr(String logicAddr) {
        ...
        String result = Integer.toBinaryString(Integer.parseInt(logicAddr.substring(0,16)+"0000",2) + Integer.parseInt(logicAddr.substring(32,48),2));
    ...
    private String toSegLinearAddr(String logicAddr) {
        // TODO: paste your code here
        int segIndex = getSegIndex(logicAddr);
        char[] baseOfSegDes = memory.getBaseOfSegDes(segIndex);
        String result = Integer.toBinaryString(Integer.parseInt(String.valueOf(baseOfSegDes),2) + Integer.parseInt(logicAddr.substring(16,48),2));
        while(result.length() < 32) result = "0" + result;
        return result;
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        // TODO: paste your code here
        // TODO: add tlb here
        int vPageNo =  Integer.parseInt(linearAddr.substring(0,20),2);
        String physicalNo;
        if(TLB.isAvailable){
            physicalNo = String.valueOf(tlb.getFrameOfPage(vPageNo));
        }else{
            physicalNo = String.valueOf(memory.getFrameOfPage(vPageNo));
        }

        return physicalNo + linearAddr.substring(20);
    }
