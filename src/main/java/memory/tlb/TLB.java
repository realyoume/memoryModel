...
public class TLB {
    ...
    public boolean isInTlb(int vPageNo){
        for(int i=0; i<TLB_SIZE; i++){
            if(getTLBLine(i).vPageNo == vPageNo && getTLBLine(i).valid) return true;
        }
        return false;
    }
    ...
    public char[] getFrameOfPage(int vPageNo) {
        for(int i=0; i<TLB_SIZE; i++){
            if(getTLBLine(i).vPageNo == vPageNo) return getTLBLine(i).pageFrame;
        }
        return null;
    }
